...
/// - All three accept share_cache_from to reuse an existing PivotTable's PivotCache. Required when
///   several PivotTables must be filtered by one slicer (slicer 'connect-pivots').
///
/// MULTI-TABLE SLICER LINK (3 steps):
/// 1. Create the first PivotTable normally.
/// 2. Create every other PivotTable with share_cache_from set to that first PivotTable, so all of
///    them sit on one PivotCache.
/// 3. Create the slicer on any of them, then call slicer 'connect-pivots' for the remaining ones.
/// A slicer can only filter PivotTables that share one PivotCache, so step 2 is not optional.
/// </summary>
[ServiceCategory("pivottable", "PivotTable")]
[McpTool("pivottable", Title = "PivotTable Operations", Destructive = true, Category = "analysis",
    Description = "PivotTable lifecycle: create from various sources, list, read, refresh, delete. BEST PRACTICE: Use list before creating. Prefer refresh over delete+recreate to preserve field configs. REFRESH: Call after configuring fields with pivottable_field. LAYOUT: 0=Compact (default), 1=Tabular (best for export), 2=Outline. CREATE: create-from-range, create-from-table, create-from-datamodel. SHARED CACHE: to filter several PivotTables with one slicer, create the first PivotTable normally, create the others with share_cache_from=<that PivotTable>, then call slicer connect-pivots for each remaining PivotTable - a slicer only filters PivotTables that share one PivotCache. TIMEOUT: 5 min for DataModel. STYLING: PivotTable visual styles are not supported by this API. Do not apply range_format to PivotTable cells — cell formatting is overwritten on the next refresh. Use pivottable_field for field management, pivottable_calc for calculated fields.")]
public interface IPivotTableCommands
{
    ...
    /// <param name="shareCacheFrom">
    /// Optional name of an existing PivotTable. When supplied, the new PivotTable reuses that
    /// PivotTable's PivotCache instead of creating its own. PivotTables sharing one PivotCache can
    /// all be driven by the same slicer (see the slicer 'connect-pivots' action). Omit to create
    /// an independent cache, which is the historical behaviour.
    /// For range sources the reused cache's source is verified and a mismatch is rejected with an
    /// error naming both references, because such PivotTables could never be linked by a slicer.
    /// ListObject and Data Model sources skip that check - their SourceData notation is not
    /// predictable enough to compare safely.
    /// </param>
    ...
    /// <param name="pivotTableName">Name for the new PivotTable</param>
    /// <param name="shareCacheFrom">
    /// Optional name of an existing PivotTable whose PivotCache should be reused so the two can be
    /// driven by one slicer. Omit to create an independent cache.
    /// </param>
