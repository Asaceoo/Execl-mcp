        ...
        return batch.Execute((ctx, ct) =>
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    try
                    {
                        shape = ChartCreationPath.CreateShape(
                            shapes, chartType, finalLeft, finalTop, finalWidth, finalHeight, sourceRangeObj);
                    }
                    catch (System.Runtime.InteropServices.COMException ex)
                        when (ex.HResult == unchecked((int)0x800A03EC))
                    {
                    ...
                    chart = shape.Chart;
                    ChartCreationPath.VerifyChartType(shape, chartType, sheetName);
                }
                finally
                {
        ...
        string? targetRange = null)
    {
        return batch.Execute((ctx, ct) =>
        {
            ...
            try
            {
                ...
                // Pick the creation path that matches the chart type (see ChartCreationPath).
                shape = ChartCreationPath.CreateShape(
                    shapes, chartType, finalLeft, finalTop, finalWidth, finalHeight, tableRange);
