        ...
        var mcpBuilder = builder.Services
            .AddMcpServer(options =>
            {
                ...
                options.ServerInstructions = """
                    jyyj-mcp 助手 (a localised downstream build of ExcelMcp) automates Microsoft Excel via COM interop.
            ...
            {
                // Order matters: identity validation first (it can produce its own precise error),
                // then the transparency filter that turns any remaining binder failure into text.
                filters.AddCallToolFilter(SessionIdentityFilter.Wrap);
                filters.AddCallToolFilter(ToolErrorSurfaceFilter.Wrap);
            })
...
#pragma warning restore CA1031
    ...
    internal static string BuildHelpText()
    {
        ...
        return $"""
            ...
            Source:
              https://github.com/Asaceoo/Execl-mcp

            Based on ExcelMcp by Stefan Broenner (MIT):
    ...
    private static async Task ShowVersionAsync()
    {
        ...
        Console.WriteLine($"jyyj-mcp 助手 v{currentVersion}");
