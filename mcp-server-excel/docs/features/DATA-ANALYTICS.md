...
- **Share Cache (`share_cache_from`):** Optional. Point a new PivotTable at an existing PivotTable's PivotCache. PivotTables sharing one PivotCache are the only ones a single slicer can filter together — see `slicer` `connect-pivots`.
