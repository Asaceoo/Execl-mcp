...
- **Connect Pivots:** Make one slicer filter several PivotTables at once
- **Disconnect Pivots:** Stop a slicer from filtering a PivotTable (keeps at least one connection)
...
- **Multi-PivotTable filtering:** a slicer can only drive PivotTables that share one PivotCache. Build the extra PivotTables with `pivottable` `share_cache_from`, then call `slicer` `connect-pivots`. Excel rejects the connection with a bare COM error otherwise, so `connect-pivots` re-reads the slicer's connections and names the PivotTables it refused.
