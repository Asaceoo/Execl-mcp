...
public partial class PivotTableCommandsTests
{
    ...
    public void GroupByDate_MonthsInterval_CreatesMonthlyGroups()
    {
        ...
        // Excel creates a month-grouped field when grouping by months (name localised per Office UI language)
        var hasMonthsField = HasGroupedDateField(listResult.Fields, "Month", "月");
    ...
    public void GroupByDate_DaysInterval_CreatesDailyGroups()
    {
        ...
        // Excel creates a day-grouped field when grouping by days (name localised per Office UI language)
        var hasDaysField = HasGroupedDateField(listResult.Fields, "Day", "天");
    ...
    public void GroupByDate_QuartersInterval_CreatesQuarterlyGroups()
    {
        ...
        // Excel creates a quarter-grouped field when grouping by quarters (name localised per Office UI language)
        var hasQuartersField = HasGroupedDateField(listResult.Fields, "Quarter", "季度");
    ...
    public void GroupByDate_YearsInterval_CreatesYearlyGroups()
    {
        ...
        // Excel creates a year-grouped field when grouping by years (name localised per Office UI language)
        var hasYearsField = HasGroupedDateField(listResult.Fields, "Year", "年");
    ...
    /// <summary>
    /// Excel names a date-group field after its grouping interval and localises that name: an
    /// English install produces "Months", a Chinese install produces "月(Date)". Asserting on the
    /// English token alone makes these tests red on any non-English Office install even when the
    /// grouping worked, so accept either form.
    /// </summary>
    private static bool HasGroupedDateField(List<PivotFieldInfo>? fields, string englishToken, string localizedToken)
        => fields != null && fields.Any(f => f.Name != null
            && (f.Name.Contains(englishToken, StringComparison.OrdinalIgnoreCase)
                || f.Name.Contains(localizedToken, StringComparison.Ordinal)));
