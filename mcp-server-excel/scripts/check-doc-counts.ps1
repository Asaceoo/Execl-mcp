...
# ---------------------------------------------------------------------------
# 6. Distribution metadata must link to the canonical project URL
# ---------------------------------------------------------------------------
# Package/registry listing pages are the highest-authority inbound links we control.
# This downstream build has no documentation site of its own, so the canonical URL is the
# jyyj-mcp repository. All four must agree - a half-renamed project is worse than either name.
$siteUrl = 'https://github.com/Asaceoo/Execl-mcp'
$siteLinkChecks = @(
    @{ File = "Directory.Build.props";                  Pattern = '<PackageProjectUrl>https://github\.com/Asaceoo/Execl-mcp</PackageProjectUrl>';  What = "PackageProjectUrl (NuGet package pages)" }
    @{ File = "src\ExcelMcp.McpServer\.mcp\server.json"; Pattern = '"websiteUrl"\s*:\s*"https://github\.com/Asaceoo/Execl-mcp"';                    What = "websiteUrl (MCP registry listing)" }
    @{ File = "mcpb\manifest.json";                     Pattern = '"homepage"\s*:\s*"https://github\.com/Asaceoo/Execl-mcp"';                      What = "homepage (Claude Desktop bundle)" }
    @{ File = "vscode-extension\package.json";          Pattern = '"homepage"\s*:\s*"https://github\.com/Asaceoo/Execl-mcp"';                      What = "homepage (VS Code Marketplace)" }
)
...
foreach ($check in $siteLinkChecks) {
    ...
    if ((Get-Content $path -Raw) -notmatch $check.Pattern) {
        Add-Failure "$($check.File): $($check.What) must point at $siteUrl - this is a primary inbound link and must not regress to the upstream URL."
    }
}
